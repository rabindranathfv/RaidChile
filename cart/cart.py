...
class Cart(object):
	...
	def __init__(self, request):
		...
		if not cart:
			cart = self.session[settings.CART_SESSION_ID] = {'items': {}, 'combo_id': None}
		self.cart = cart
	...
	def add(self, product, adult_qty=1, children_qty=0, update_quantity=False):
		...
		# Is the newly added item a combo?
		if product.product_type == 'COMBO' and self.cart['combo_id'] == None:
			self.cart['combo_id'] = product.id
	...
	def remove(self, product):
		...
		if product_id in self.cart['items']:
			# If the deleted product is a combo, reset the combo_id
